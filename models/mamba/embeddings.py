...
class SensorEmbeddingLayer(nn.Module):
    ...
    def __init__(self, num_sensor: int, embedding_size: int, dim_embedding: int):
        ...
        self.sensor_embedding = nn.Linear(embedding_size, dim_embedding)
...
class ConceptEmbedding(nn.Module):  # TODO
...
class MambaEmbeddingsForCEHR(nn.Module): # TODO
    """Construct the embeddings from concept, token_type, etc., embeddings."""
    """The actual numerical values (e.g., lab results, vitals) would be represented by the concept_ids and other input tensors,
      which are then embedded into a higher-dimensional space for use in the model."""
    ...
    def __init__(
        ...
        self.word_embeddings = nn.Embedding(  # every sensor concept (like albumin for the physionet12) has to be embedded in this layer
            config.vocab_size,
            config.hidden_size,
            padding_idx=config.pad_token_id,
        )
    ...
    def forward(
        ...
        visit_segments : torch.Tensor
            Visit segments of the input data.
        """
        if inputs_embeds is None: # if the embeddings are not provided, we get them from the input_ids
            inputs_embeds = self.word_embeddings(input_ids)
        ...
        inputs_embeds = torch.cat( # concatenate the embeddings on the last dimension which is the dimension of the embeddings
            (inputs_embeds, time_stamps_embeds, ages_embeds),
            dim=-1,
        ) # input_embeds is of dimension (batch_size, sequence_length, dim_input_embeddings + dim_time_embeddings + dim_age_embeddings) 
